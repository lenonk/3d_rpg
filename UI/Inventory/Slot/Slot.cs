...
public partial class Slot : Panel {
	...
	private void SetupHoverPanel() {
		if (_dragging || _hoverPanel is null) return;

		var item = _player.GetInventory().At(Index);
	...
	private void OnSlotMouseEntered() {
		if (_dragging || _player.GetInventory().At(Index) is null) return;
	...
	public override async void _DropData(Vector2 atPosition, Variant data) {
		...
		_dragNumber = 0;
		if (Input.IsActionPressed("ui_shift") && _player.GetInventory().At(rIndex).IsStackable()) {
			await ShowDragDialog();
			_dragNumber = Math.Max(0, Math.Min(_dragNumber, _player.GetInventory().At(rIndex).Count));
			if (_dragNumber <= 0) return;
		}	
		
		_player.GetInventory().MoveItem(rIndex, Index, _dragNumber);	
	}
	...
	public override Variant _GetDragData(Vector2 atPosition) {
		if (_player.GetInventory().At(Index) is null) return new();
